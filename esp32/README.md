...
`POST /readings/ingest` — identifies the device by `device_id` in the JSON body. No
token or session is required; the device must exist and be active.

## Setup

1. **Create the device** in the Aqua AI web app (Devices page → Add Device) and note
   the device ID it returns.
...
- Readings are sent every `SEND_INTERVAL_MS` (default 60 s) using a non-blocking
  `millis()` cadence.

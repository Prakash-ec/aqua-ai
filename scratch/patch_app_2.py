...
new_update_analysis = """function updateAnalysisPage() {
    ...
    if (!latestReading) {
        ...
        ['analysisSummary','analysisParameterAssessment','analysisKeyLimitingFactor','analysisCropSection','analysisIndustrialSection','analysisDomesticSection','analysisGeneralSection','analysisDrinkingSection'].forEach(function(id) { setAnalysisHtml(id, ''); });
        setAnalysisNotice('');
        return;
    }
    ...
    var drinkHtml = `
        <div style="background: var(--bg-soft); border-radius: 8px; padding: 16px; border: 1px solid var(--border-light);">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                ...
                <span style="font-size: 16px; font-weight: 600; color: ${drinking.score >= 80 ? 'var(--status-normal)' : (drinking.score >= 50 ? 'var(--status-monitor)' : 'var(--status-critical)')};">${drinking.score !== null ? Math.round(drinking.score) : '--'} / 100</span>
            </div>
...
function renderCompactRow(name, score, reason) {
    ...
    return `
        <div style="display: flex; flex-direction: column; gap: 4px; padding: 12px; background: var(--bg-soft); border-radius: 6px; border: 1px solid var(--border-light);">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <strong style="font-size: 14px; color: var(--text-primary);">${name}</strong>
                <span style="font-weight: 600; font-size: 14px; color: ${color};">${sVal}</span>
            </div>
...
# replace updateAnalysisPage completely

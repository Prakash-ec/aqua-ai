...
function renderDetailedApplicationCard(item, idPrefix, icon) {
    ...
    if (type === 'agriculture') {
        ...
        salReason = salScore < 100 && isHigh ? "EC is above the reference \u2192 salinity score is reduced." : (salScore === 100 ? "EC is within optimal range." : "EC affects score.");
    } else {
        ...
        salReason = salScore < 100 && isHighTDS ? "TDS is above the reference \u2192 salinity score is reduced." : (salScore === 100 ? "TDS is within optimal range." : "TDS affects score.");
    }
    
    var scores = [
        { name: 'pH', score: phScore },
        { name: salLabel, score: salScore },
    ...
    var rowFmt = function(label, cur, ref, sc) {
        return '<div style="display: flex; justify-content: space-between; font-size: 12px; padding: 4px 0; border-bottom: 1px solid var(--border-subtle);">' +
            ...
            '<span style="flex: 1; text-align: right; font-weight: 600; color: ' + getQualityClass(sc).color + ';">' + (sc !== null ? Math.round(sc) + '%' : '--') + '</span>' +
            '</div>';
    };
...
function updateAnalysisPage(reading) {
    if (!reading) return;
    hideAnalysisNotice();
    ...
    setAnalysisHtml('analysisKeyLimitingFactor', ''); // Clear limiting factor global text to keep it clean
    ...
    var dhHtml = '<div style="background: var(--bg-soft); border-radius: 12px; padding: 20px;">' +
        ...
        '<div style="display: flex; justify-content: space-between; font-size: 12px; padding: 6px 0; border-bottom: 1px solid var(--border-subtle);"><span style="flex: 1;">pH</span><span style="flex: 1; text-align: right; color: var(--text-secondary);">' + fmt(reading.ph, 2) + '</span><span style="flex: 1; text-align: right; color: var(--text-secondary);">' + dr.phMin + '-' + dr.phMax + '</span><span style="flex: 1; text-align: right; font-weight: 600;">' + Math.round(dRes.phScore) + '%</span></div>' +
        '<div style="display: flex; justify-content: space-between; font-size: 12px; padding: 6px 0; border-bottom: 1px solid var(--border-subtle);"><span style="flex: 1;">TDS</span><span style="flex: 1; text-align: right; color: var(--text-secondary);">' + fmt(reading.tds, 0) + '</span><span style="flex: 1; text-align: right; color: var(--text-secondary);">' + dr.tdsMaximum + ' max</span><span style="flex: 1; text-align: right; font-weight: 600;">' + Math.round(dRes.tdsScore) + '%</span></div>' +
        '<div style="display: flex; justify-content: space-between; font-size: 12px; padding: 6px 0;"><span style="flex: 1;">Turbidity</span><span style="flex: 1; text-align: right; color: var(--text-secondary);">' + fmt(reading.turbidity, 2) + '</span><span style="flex: 1; text-align: right; color: var(--text-secondary);">' + dr.turbidityMaximum + ' max</span><span style="flex: 1; text-align: right; font-weight: 600;">' + Math.round(dRes.turbidityScore) + '%</span></div>' +
...
text = re.sub(r'function updateAnalysisPage\(reading\) \{[\s\S]*?(?=\nfunction setAnalysisHtml)', js_code, text)
